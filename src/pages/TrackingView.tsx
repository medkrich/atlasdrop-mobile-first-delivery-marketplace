...
export function TrackingView() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center">
      <div className="w-full max-w-md h-screen relative flex flex-col overflow-hidden bg-white shadow-2xl">
        ...
        <div className="flex-1 pt-24 px-6 bg-slate-50">
          <TrackingMap 
        ...
        {/* Stable Non-Portal Panel replaces unstable Vaul Root for the main UI */}
        <motion.div 
          initial={{ y: 100, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          className="bg-white rounded-t-[32px] shadow-[0_-10px_40px_rgba(0,0,0,0.1)] z-30 p-6 space-y-6 pb-10"
        >
          ...
          <AnimatePresence mode="wait">
            {delivery.status !== 'delivered' ? (
              <motion.div 
                ...
                className="p-4 bg-orange-50 rounded-2xl flex items-center gap-4 border border-orange-100"
              >
              ...
              <motion.div 
                ...
                className="flex flex-col items-center py-4 text-center space-y-2"
              >
          ...
          {canProgress && (
            <Button 
              ...
              className="w-full h-14 bg-[#004E89] hover:bg-[#003d6d] rounded-2xl text-lg font-bold shadow-lg transition-all active:scale-[0.98]"
            >
              {getActionButtonText()}
            </Button>
          )}
        </motion.div>
        {/* Modal Chat Drawer remains stable as it uses its own Portal logic */}
        {id && (
          <ChatDrawer 
            deliveryId={id} 
            isOpen={isChatOpen} 
